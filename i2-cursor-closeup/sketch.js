...
let mainSketch = function(p) {
  ...
  p.setup = function() {
    ...
    outerStroke = p.width/50
    innerStroke = p.width/90
...
let cursorSketch = function(p) {
  ...
  p.setup = function() {
      canvas = p.createCanvas(1600,1600);  // Create a canvas
  ...
  p.draw = function() {
      ...
      if (p.mouseX != 0) {
        let cursorScale = .15
        p.image(imgCursor, p.mouseX, p.mouseY ,p.width * cursorScale * .16, p.width * cursorScale * .22);  // Draw the image at the mouse location
      }
  };
};
  ...
  for (let i = start; i < end; i++) {
    for (let j = 0; j < easyPoints.length; j++) {
      if (easyPoints[j].zIndex == i) {
        ...
        p.stroke(strokeColor)
        p.strokeWeight(outerStroke)
        drawSegment(j, p)
        p.stroke(bgColor)
        // stroke(colors[easyPoints[j].grp])
        p.strokeWeight(innerStroke)
        drawSegment(j, p)
      }
    }
  }
